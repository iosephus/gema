...
(defn check-model-pars
  ...
  [simul-info pars]
  (if (contains? pars :r) (> (double (pars :r)) 0.0) true))

(defn build-model-info
  "Returns map with model info from input parameters"
  [simul-info pars]
  (let 
    [r (if (contains? pars :r) (double (pars :r)) 1.0) 
     r2 (* r r)] 
    {:dim 3 :r r :r2 r2}))
...
(defn get-initpos
  ...
  (utils/get-rand-3 (model-pdfs :init-pdf)))
...
(defn check-initpos
  ...
  (check-displacement simul-info model-info {:step 0 :state 0 :pos (double-array 3 0.0)} pos pos))
